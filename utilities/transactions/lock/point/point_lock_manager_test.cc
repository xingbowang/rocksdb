...
constexpr auto kLongTxnTimeoutUs = 1000000;
...
TEST_F(PointLockManagerTest, LockEfficiency) {
  ...
  for (int i = 0; i < num_of_txn; i++) {
    ...
    if (i == 0) {
      ...
      locker_->TryLock(txn, 1, "k1", env_, true);
    } else {
...
TEST_F(PointLockManagerTest, LockFairness) {
  ...
  for (int i = 0; i < num_of_txn; i++) {
    ...
    if (i == 0) {
      ...
      locker_->TryLock(txn, 1, "k1", env_, txn_lock_types[0]);
    } else {
...
TEST_F(PointLockManagerTest, LockStealAfterTimeoutExclusive) {
  ...
  // order is respected.
  // txn2 is waked up and takes the lock. unlock txn2, txn3 should proceed.
  ...
  // validate txn2 is waked up and takes the lock
  t1.join();
...
TEST_F(PointLockManagerTest, RandomLockAcquireRelease) {
    ...
    threads.emplace_back([this, &txn_opt, &shutdown, &num_of_locks_acquired]() {
      ...
      while (!shutdown) {
        ...
        for (uint32_t j = 0; j < num_key_to_lock; j++) {
          ...
          auto it = locked_key_with_types.begin();
          while (it != locked_key_with_types.end()) {
            if (it->first == key) {
              break;
            }
            it++;
          ...
          if (s.ok()) {
            DEBUG_LOG("Txn %lu acquired lock %s type %s\n", txn->GetID(),
                      key.c_str(), lock_type ? "exclusive" : "shared");
            num_of_locks_acquired++;
            locked_key_with_types.emplace_back(key, lock_type);
          } else {
            if (s.IsDeadlock()) {
              DEBUG_LOG("Txn %lu detected deadlock on key %s, abort\n",
                        txn->GetID(), key.c_str());
              break;
            }
            DEBUG_LOG(
                "Txn %lu failed to acquire lock on key %s due to "
                "%s, retry\n",
                txn->GetID(), key.c_str(), s.ToString().c_str());
        ...
        if (!s.IsDeadlock()) {
          ...
          // expiration time to pretend to do some work.
          auto sleep_time_us =
        ...
        for (auto& key_type : locked_key_with_types) {
          ...
          DEBUG_LOG("Txn %lu release lock %s\n", txn->GetID(),
