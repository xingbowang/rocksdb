        ...
        if (must_not_exist || !in_expected_value_base_range) {
          ...
          fprintf(
              ...
              static_cast<unsigned>(
                  pre_read_expected_value.GetFinalValueBase()),
              static_cast<unsigned>(pre_read_expected_value.GetDelCounter()),
              static_cast<unsigned>(
                  pre_read_expected_value.GetFinalDelCounter()),
          ...
          auto dump_debug_iter = [&](const char* label,
            ...
            fprintf(
                stderr,
                "%s (%s): valid=%d, status=%s, sv=%s, key=%s, value=%s\n",
                ...
                debug_iter->Valid() ? debug_iter->key().ToString(true).c_str()
                                    : "(invalid)",
                (debug_iter->Valid() && (!debug_ro.allow_unprepared_value ||
                                         debug_iter->status().ok()))
