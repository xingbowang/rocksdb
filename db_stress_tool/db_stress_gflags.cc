...
DEFINE_string(stress_diagnostics_dir, "",
              ...
              "If empty, diagnostics that require file output are not flushed "
              "to disk.");

DEFINE_bool(stress_diagnostics_breadcrumbs, false,
            "If true, keep a bounded per-thread ring of recent stress "
            "operations and periodically write it to "
            "--stress_diagnostics_dir when set.");
...
DEFINE_uint64(stress_diagnostics_breadcrumb_flush_every, 1000,
              "Flush each thread's operation breadcrumb ring to "
              "--stress_diagnostics_dir every N completed operation scopes. "
              "A zero value disables periodic flushing; verification failures "
              "still trigger a best-effort flush.");
