...
class PinnableKeyValue {
 public:
  ...
  // True if key() aliases retained backing storage instead of the internal
  // copy buffer.
  bool IsKeyPinned() const { return key_.IsPinned(); }
  // True if value() aliases retained backing storage instead of the internal
  // copy buffer.
...
// Rows that share the same retained backing storage are accounted once in
// GetPinnedBlockCacheUsage().
class PinnableKeyValueBatch {
 public:
  ...
  // True if key(index) aliases retained backing storage instead of the
  ...
  // True if value(index) aliases retained backing storage instead of the
  // internal copy buffer.
...
class Iterator : public IteratorBase {
 public:
  ...
  //
  // REQUIRES: Valid()
  // Some row types and iterator states are not yet supported and return
  // NotSupported.
  ...
  // valid until `batch->Reset()` or object destruction. On error, previously
  // appended rows in `batch` are left intact.
  //
  // REQUIRES: Valid()
  // Some row types and iterator states are not yet supported and return
  // NotSupported.
