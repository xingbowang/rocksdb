    ...
    void LoadCurrentRecord() {
      ...
      if (rexdb_udb_iterator_arg_ &&
        ...
        for (uint32_t idx = current_record_idx_; idx < records_.size(); idx++) {
          ...
          std::string key_str = UserKey(record.first).ToString();
   ...
   public:
    ...
    DataBlockIter* NewDataIterator(
      ...
      if (input_iter != nullptr &&
        ...
        iter = static_cast<RexDBBlockIterator*>(input_iter);
      } else {
...
class MyFlushListener : public EventListener {
 public:
  void OnFlushCompleted(DB* db, const FlushJobInfo& info) override {
    auto lmax = 6;
    // Call compactFiles to move the new flushed file from L0 to Lmax
    ASSERT_OK(db->CompactFiles(CompactionOptions(), {info.file_path}, lmax));
  }
};

// Verifies a RexDB-style custom block prototype by writing column-like values,
// scanning full rows, then using the iterator arg to collect projected columns.
TEST_F(RexDB, RexDBPrototypeTest) {
  ...
  // Block-owned slices only remain valid while the iterator is on that block,
  // so the test copies projected values into the request arg before advancing.
  ...
  // Verify the iterator collected projected rows before the upper bound.
