...
struct PinnedIterKeyValue {
  // `key` is the internal key backing the current row. When `key_pinned` is
  // true, `user_key` aliases the retained backing storage for the exposed user
  // key. When false, callers must copy from their current Iterator::key().
  Slice key;
  Slice user_key;
  Slice value;
