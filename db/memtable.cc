...
Status MemTable::BatchAdd(const MemTableEntry* entries, size_t num_entries,
  ...
  for (size_t i = 0; i < num_entries; ++i) {
    ...
    // Select the appropriate table (point keys vs range deletions)
    std::unique_ptr<MemTableRep>* current_table =
        (entry.type == kTypeRangeDeletion) ? &range_del_table_ : &table_;

    // For simplicity, require all entries to use the same table in this batch
    if (table == nullptr) {
      table = current_table;
    ...
    // Format: key_size + key_bytes + value_size + value_bytes + checksum
  ...
  // Insert all entries at once using batch insert
  ...
  if (post_process_info == nullptr) {
    // Non-concurrent path: update directly
    ...
    // Concurrent path: accumulate in post_process_info
  ...
  // Update bloom filters and sequence numbers for all entries
  for (size_t i = 0; i < num_entries; ++i) {
    const MemTableEntry& entry = entries[i];
  ...
  // Handle range deletion cache invalidation if needed
  if (num_range_del > 0) {
    auto new_cache = std::make_shared<FragmentedRangeTombstoneListCache>();
    size_t size = cached_range_tombstone_.Size();
    range_del_mutex_.lock();
