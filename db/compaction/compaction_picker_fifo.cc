...
uint64_t EstimateTotalDataForSST(uint64_t sst_bytes,
                                  uint64_t total_sst,
                                  uint64_t total_blob) {
  if (total_sst == 0 || total_blob == 0) {
    return sst_bytes;
  }
  return sst_bytes + static_cast<uint64_t>(
      static_cast<double>(sst_bytes) / total_sst * total_blob);
}
...
void GetEffectiveSizeAndLimit(
    const CompactionOptionsFIFO& fifo_opts,
    uint64_t total_sst_size, uint64_t total_blob_size,
    uint64_t* effective_size, uint64_t* effective_max) {
...
uint64_t GetEffectiveMax(const CompactionOptionsFIFO& fifo_opts) {
  return fifo_opts.max_data_files_size > 0
      ? fifo_opts.max_data_files_size
...
Compaction* FIFOCompactionPicker::PickTTLCompaction(
  ...
  if (mutable_cf_options.compaction_options_fifo.max_data_files_size > 0) {
    ...
    effective_remaining = EstimateTotalDataForSST(
        total_size, original_sst_size, total_blob);
  }
...
Compaction* FIFOCompactionPicker::PickSizeCompaction(
  ...
  if (last_level == 0 && effective_size <= effective_max) {
    ROCKS_LOG_BUFFER(
        ...
        ", max size %" PRIu64 "\n",
        cf_name.c_str(), effective_size, effective_max);
    return nullptr;
  }
  ...
  if (last_level == 0) {
    ...
    for (const auto& f : last_level_files) {
      ...
      ROCKS_LOG_BUFFER(
          ...
          " vs max size %" PRIu64,
          cf_name.c_str(), f->fd.GetNumber(), tmp_fsize, effective_size,
          effective_max);
...
Compaction* FIFOCompactionPicker::PickIntraL0Compaction(
  ...
  if (fifo_opts.use_kv_ratio_compaction) {
    return PickRatioBasedIntraL0Compaction(cf_name, mutable_cf_options, mutable_db_options,
                                vstorage, log_buffer);
  }
  ...
  if (PickCostBasedIntraL0Compaction(
          ...
          max_compact_bytes_per_del_file, mutable_cf_options.max_compaction_bytes,
          &comp_inputs)) {
    Compaction* c = new Compaction(
        vstorage, ioptions_, mutable_cf_options, mutable_db_options,
...
Compaction* FIFOCompactionPicker::PickRatioBasedIntraL0Compaction(
  ...
  if (mutable_cf_options.max_compaction_bytes > 0) {
    ...
    double sst_ratio = (total_blob > 0)
        ? static_cast<double>(total_sst) / total_data
        : 1.0;

    uint64_t total_sst_at_cap = static_cast<uint64_t>(
        fifo_opts.max_data_files_size * sst_ratio);
  ...
  for (const uint64_t boundary : boundaries) {
    for (size_t scan = level0_files.size(); scan > 0;) {
      ...
      size_t pos = scan;
      while (pos > 0 &&
             level0_files[pos - 1]->fd.file_size < boundary &&
      ...
      if (batch.size() >= 2 && accumulated >= boundary) {
        ...
        ROCKS_LOG_BUFFER(
            ...
            cf_name.c_str(), comp_inputs.files.size(), accumulated,
            boundary, target);
