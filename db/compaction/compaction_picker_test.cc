...
class CompactionPickerTestBase : public testing::Test {
 public:
  ...
  std::unique_ptr<Compaction> PickFIFOCompaction(
      FIFOCompactionPicker& picker) {
...
TEST_F(CompactionPickerTest, FIFORatioBasedCompactionWithExplicitMaxCompactionBytes) {
...
TEST_F(CompactionPickerTest, FIFOKvRatioValidationRequiresFIFO) {
  ...
  cf_opts.compaction_options_fifo.max_data_files_size = 1ULL * 1024 * 1024 * 1024;
  DBOptions db_opts;
  ASSERT_TRUE(
...
TEST_F(CompactionPickerTest, FIFOBlobAwareScoreComputation) {
  ...
  // With blob-aware sizing: score = (total_sst + total_blob) / max_data_files_size
  //
  ...
  ASSERT_GT(score, 2.0)
      << "Score should reflect 500MB/200MB ≈ 2.5";
}
...
class FIFORatioBasedCompactionPickingTest : public CompactionPickerTest {
 protected:
  ...
  std::vector<size_t> PickCompactionFromFiles(
    ...
    for (size_t j = 0; j < compaction->num_input_files(0); j++) {
      uint32_t fn = static_cast<uint32_t>(
          compaction->input(0, j)->fd.GetNumber());
  ...
  void RunFlushAndCompact(TestState& s, int num_rounds,
                     int trigger,
                     uint64_t max_data_files_size,
    ...
    for (int round = 0; round < num_rounds; round++) {
      ...
      auto inputs = PickCompactionFromFiles(picker, s.files,
                                       max_table_files_size,
                                       max_data_files_size, trigger,
                                       &reason);
      if (!inputs.empty()) {
  ...
  void AssertCompactedUniform(const std::vector<L0File>& files,
                              double max_cv) {
  ...
  void AssertGraduatedNotPicked(const std::vector<L0File>& files,
                                int trigger,
                                uint64_t max_data_files_size) {
    ioptions_.compaction_style = kCompactionStyleFIFO;
    FIFOCompactionPicker picker(ioptions_, &icmp_);
    ...
    CompactionReason reason;
    auto inputs = PickCompactionFromFiles(picker, files,
                                     max_table_files_size,
                                     max_data_files_size, trigger,
                                     &reason);
    if (!inputs.empty() &&
        reason == CompactionReason::kFIFOReduceNumFiles) {
      ...
      double sst_ratio = total_blob > 0
          ? static_cast<double>(total_sst) / (total_sst + total_blob)
          : 1.0;
      uint64_t target = static_cast<uint64_t>(
          max_data_files_size * sst_ratio) / trigger;
...
TEST_F(FIFORatioBasedCompactionPickingTest, VariableFlushWithFIFODropping) {
  ...
  Random rng(42);
  TestState s;
  RunFlushAndCompact(s, 200, /*trigger=*/10, kCap,
                [&](int) {
...
TEST_F(FIFORatioBasedCompactionPickingTest, NoCascadingReCompaction) {
  ...
  TestState s;
  RunFlushAndCompact(s, 200, /*trigger=*/10, kCap,
                [](int) { return std::make_pair(64ULL * 1024, 64ULL * 1024 * 1024); });
...
TEST_F(FIFORatioBasedCompactionPickingTest, EarlyMemtableFlush) {
  ...
  Random rng(123);
  TestState s;
  RunFlushAndCompact(s, 100, /*trigger=*/10, kCap,
                [&](int) {
                  uint64_t sst = (rng.Next() % 5 == 0)
                      ? (64 + rng.Next() % 65) * 1024ULL
...
TEST_F(FIFORatioBasedCompactionPickingTest, BlobCompressionVariation) {
  ...
  Random rng(456);
  TestState s;
  RunFlushAndCompact(s, 150, /*trigger=*/10, kCap,
                [&](int) {
...
TEST_F(FIFORatioBasedCompactionPickingTest, TieredLargeRatio) {
  ...
  RunFlushAndCompact(s, 500, /*trigger=*/10, kCap,
                [](int) { return std::make_pair(1ULL * 1024,
                                                1ULL * 1024 * 1024); });
...
TEST_F(FIFORatioBasedCompactionPickingTest, TieredProgression) {
  ...
  TestState s;
  RunFlushAndCompact(s, 200, /*trigger=*/4, kCap,
                [](int) { return std::make_pair(10ULL * 1024,
                                                1ULL * 1024 * 1024); });
...
TEST_F(FIFORatioBasedCompactionPickingTest, GraduatedFilesNotRecompacted) {
  ...
  RunFlushAndCompact(s, 60, /*trigger=*/4, kCap,
                [](int) { return std::make_pair(64ULL * 1024,
                                                50ULL * 1024 * 1024); });
