...
// Verifies PinCurrent() copies a delta-encoded key while keeping the current
// SST value pinned from block cache after the iterator moves and is destroyed.
// The small block size forces multiple data blocks so the pinned row must own
// its own lifetime instead of relying on the iterator's current block.
TEST_F(DBTest2, IteratorPinCurrentDeltaEncodedKeyAndPinnedValue) {
  ...
  ASSERT_EQ(pinned_usage_before_reset - row_pinned_usage,
            pinned_usage_after_reset);
}

// Verifies both pin APIs keep a non-delta-encoded SST row fully zero-copy.
// Disabling delta encoding makes the user key directly alias the cached block,
// so both key and value should be pinned with no copied bytes.
...
// Verifies PinCurrent() still succeeds on plain SST values when block-cache
// pinning is unavailable. The iterator falls back to copying both key and
// value into PinnableKeyValue so the row remains valid after iterator cleanup.
...
// Verifies AppendPinnedCurrent() appends multiple delta-encoded keys from the
// same data block while retaining a single block-cache lease for the whole
// batch. The batch stays valid after iterator movement and destruction, and
// Reset() releases exactly that one retained block charge.
TEST_F(DBTest2, IteratorAppendPinnedCurrentDedupesSameBlockLease) {
  ...
  batch.Reset();
  ASSERT_EQ(pinned_usage_before_reset - batch_pinned_usage,
            block_cache->GetPinnedUsage());
}

// Verifies AppendPinnedCurrent() accumulates pinned usage across different
// data blocks instead of collapsing all rows into one lease. The sparse seeks
// land on distant rows in a multi-block SST so the batch must retain multiple
// block-cache entries and charge both.
...
// Verifies AppendPinnedCurrent() keeps working when block-cache pinning is
// unavailable. The batch copies both key and value for each appended row, so
// the collected rows stay valid after iterator destruction without any pinned
...
// Verifies PinCurrent() rejects reverse iteration and clears any previously
// pinned output. A failed pin must not leave stale slices or stale memory
// accounting behind in the caller-owned handle.
...
// Verifies reverse iteration stays unsupported for both pin APIs and that the
// failure mode is safe for caller-owned outputs. PinCurrent() must clear the
// row handle, while AppendPinnedCurrent() must leave the existing batch
// untouched when the iterator is positioned in reverse mode.
...
// Verifies Reset() releases the retained block-cache lease and clears all
// accounting so the batch can be reused for a later row without mixing old and
// new state. This protects the common RPC-buffering pattern of fill, send,
// reset, and refill.
...
// Verifies the public API validates output pointers before touching iterator
// state. Both pin methods should return InvalidArgument when passed nullptr.
...
// Verifies blob-backed rows remain unsupported even if the caller explicitly
// materializes the value first. This exercises the allow_unprepared_value path
// and pins the contract that v1 still rejects kTypeBlobIndex rows.
...
// Verifies wide-column entities are explicitly rejected by the new pin APIs in
// v1. The iterator can still expose the default column through value(), but the
// underlying row type is not yet supported for row pinning.
...
// Verifies user-defined timestamp reads are rejected up front. The iterator is
// valid and can read the row normally, but PinCurrent()/AppendPinnedCurrent()
// must still return NotSupported while timestamp-aware iteration is out of
