...
// Simple benchmark comparing individual Add vs BatchAdd performance
class BatchAddBenchmark {
 ...
 private:
  int64_t RunWithAdd(bool use_batch_add, bool random_order) {
    ...
    options.allow_concurrent_memtable_write = false;  // Simplify for benchmark
    ...
    // Shuffle keys if random order is requested
    if (random_order) {
      ...
      // Reorder keys and values based on shuffled indices
    ...
    write_options.disableWAL =
        true;  // Disable WAL to focus on memtable performance
    write_options.use_batch_add = use_batch_add;

    auto start = std::chrono::high_resolution_clock::now();

    // Insert entries using WriteBatch
    const int batch_size = 1000;  // Insert 1000 entries per WriteBatch
