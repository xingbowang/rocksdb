...
void PinnableWideColumns::Move(PinnableWideColumns&& other) {
  // When SetWideColumnValueLazy is used (lazy resolution path), the resolver
  // owns its data via ResetWithOwnedData(), so the pointers it holds remain
  // valid after the move. For the SetWideColumnValue path (eager resolution),
  // resolver_ is always nullptr.
