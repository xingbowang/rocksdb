...
Status WideColumnSerialization::SerializeWithBlobIndices(
  ...
  std::vector<std::pair<size_t, std::string>> serialized_blob_indices;
  serialized_blob_indices.reserve(blob_columns.size());
  std::vector<uint32_t> name_sizes(num_columns);
  std::vector<uint32_t> value_sizes(num_columns);
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    if (blob_index_map[i] != nullptr) {
      ...
      value_sizes[i] = static_cast<uint32_t>(serialized.size());
      serialized_blob_indices.emplace_back(i, std::move(serialized));
    } else {
      if (value.size() >
          static_cast<size_t>(std::numeric_limits<uint32_t>::max())) {
        return Status::InvalidArgument("Wide column value too long");
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    output->append(columns[i].name().data(), columns[i].name().size());
  }

  // Section 7: COLUMN VALUES
  // Build a quick lookup for serialized blob indices
  size_t blob_ser_idx = 0;
  for (size_t i = 0; i < num_columns; ++i) {
    if (blob_index_map[i] != nullptr) {
      assert(blob_ser_idx < serialized_blob_indices.size());
      assert(serialized_blob_indices[blob_ser_idx].first == i);
      output->append(serialized_blob_indices[blob_ser_idx].second);
      ++blob_ser_idx;
    } else {
      output->append(columns[i].value().data(), columns[i].value().size());
    }
  }
...
Status WideColumnSerialization::ResolveEntityBlobColumns(
    ...
    std::string* resolved_entity, uint64_t* total_bytes_read,
  ...
  for (const auto& blob_col : blob_columns) {
    ...
    const Status s = blob_fetcher->FetchBlob(
...
Status WideColumnSerialization::GetDefaultColumnFromEntityResolvingBlobs(
    const Slice& entity_value, const Slice& user_key,
    const BlobFetcher* blob_fetcher, PinnableSlice* result) {
  assert(blob_fetcher);
  assert(result);
